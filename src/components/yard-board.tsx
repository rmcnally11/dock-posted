...
function YardPhone({ phone }: { phone: string | null }) {
  ...
  return (
    <a
      href={href}
      className="inline-flex min-h-11 items-center text-[color:var(--diesel)] underline-offset-2 hover:underline"
    >
      {text}
    </a>
  );
    ...
    <div className="min-w-0" data-testid="yard-board">
      <ul className="space-y-3 md:hidden">
        {yards.map((yard) => (
          <li
            key={yard.id}
            className="rounded-2xl border border-[color:var(--line)] bg-white px-3 py-3"
          ...
          >
            ...
            <dl className="mt-3 grid grid-cols-2 gap-2 text-sm">
              <div>
                <dt className="text-[11px] uppercase tracking-[0.14em] text-[color:var(--ink)]/50">
                  Indoor
                </dt>
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full min-w-[36rem] text-left text-sm">
          <thead>
            <tr className="border-b border-[color:var(--line)] text-[11px] uppercase tracking-[0.14em] text-[color:var(--ink)]/50">
